...
(function () {
  ...
  const UI_TEXT = {
    zh: {
      ...
      note: "<strong>全自动提示</strong>：先双击“启动BiliFetch本地助手.bat”并保持窗口打开；不确定是否启动时，点“更多 > 检测助手”。未启动时会尝试浏览器内自动合并，不再下载 bat。",
      modeLabel: "下载内容（可多选）",
      videoLabel: "选择视频",
      qualityLabel: "清晰度",
      selectAll: "全选",
      ...
      less: "收起",
      ready: "准备好了",
      statusReady: "默认已勾选“合并视频+音频MP4”。脚本会识别所有视频，勾选要下载的视频后点“开始”。不确定本地助手是否启动时，点“更多 > 检测助手”。",
      languageLabel: "界面语言",
      modes: {
        merge: "合并视频+音频MP4",
        video: "只下载视频流",
    ...
    en: {
      ...
      note: "<strong>Automation tip</strong>: start the BiliFetch Local Helper first. If you are not sure, click More > Check Helper. If it is not running, the script tries browser-side merging and will not download a bat file.",
      ...
      preview: "Preview",
      start: "Start",
      helperStatus: "Check Helper",
      diagnose: "Copy Diagnostics",
      settings: "Settings",
      more: "More",
      less: "Less",
      ready: "Ready",
      statusReady: "Merge video + audio MP4 is selected by default. Select videos to download, then click Start. If you are not sure whether the helper is running, click More > Check Helper.",
  ...
  function installPanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="bf-body">
        <div class="bf-note"><strong>全自动提示</strong>：先双击“启动BiliFetch本地助手.bat”并保持窗口打开；不确定是否启动时，点“更多 &gt; 检测助手”。未启动时会尝试浏览器内自动合并，不再下载 bat。</div>
        <div class="bf-grid">
          <div class="bf-row">
        ...
        <div class="bf-extra-actions" id="bilifetch-scriptcat-extra-actions">
          ...
          <button class="bf-secondary" id="bilifetch-scriptcat-helper-status" type="button">检测助手</button>
        ...
        <pre id="${STATUS_ID}">默认已勾选“合并视频+音频MP4”。需要字幕、封面、信息等内容时，可继续多选后点“开始”。不确定本地助手是否启动时，点“更多 > 检测助手”。</pre>
      </div>
    `;
    ...
    panel.querySelector("#bilifetch-scriptcat-helper-status").addEventListener("click", () => {
      checkLocalHelperStatus().catch(showError);
    });
  ...
  function applyLanguage(panel = document.getElementById(PANEL_ID)) {
    ...
    setButtonText(panel, "#bilifetch-scriptcat-helper-status", text.helperStatus);
  ...
  function showChangelog(force) {
    const lines = [
      ...
      "- 新增“检测助手”，可检查本地助手、下载目录和 ffmpeg 状态。",
    ];
    if (force) {
  ...
  function diagnoseError(error) {
    ...
    if (/本地助手|127\.0\.0\.1|localhost|18991|ECONNREFUSED|无法连接本地助手|连接本地助手|helper/i.test(message)) {
      tips.push("如果要用本地助手，请先解压小白包，双击“启动BiliFetch本地助手.bat”，保持窗口打开。");
      tips.push("回到 B 站页面后点“更多 > 检测助手”，确认显示在线后再点“开始”。");
    }
    if (/ffmpeg|wasm|@require|core|SharedArrayBuffer|跨源|CORS/i.test(message)) {
      tips.push("浏览器内合并依赖 ffmpeg.wasm；如果浏览器或脚本猫拦截外部库，请先启动本地助手。");
      tips.push("只想保留手动方案时，可以勾选“导出合并命令”。");
    }
    if (/memory|内存|quota|ArrayBuffer|allocation|out of/i.test(message)) {
      tips.push("文件可能太大，浏览器内存不够。建议启动本地助手，或改用“导出合并命令”。");
    }
    ...
    if (/GM_download|下载失败|download/i.test(message)) {
      ...
      tips.push("如果浏览器下载被拦截，请在浏览器下载栏或站点权限里允许多个文件下载。");
    }
    ...
    if (/ffmpeg\.exe|ffmpeg not found|未找到 ffmpeg|not recognized/i.test(message)) {
      tips.push("本地助手合并音视频需要 ffmpeg。请把 ffmpeg.exe 放到本地助手同目录，或安装 ffmpeg 并加入 PATH。");
      tips.push("打开本地助手进度页后，顶部会显示 ffmpeg 状态。");
    }
    ...
    if (/subtitle|字幕|danmaku|弹幕|xml|json/i.test(message)) {
      tips.push("如果只是字幕、弹幕或信息文件失败，可以先取消对应勾选，只测试“合并视频+音频MP4”。");
    }
    if (!tips.length) {
  ...
  async function checkLocalHelperStatus() {
    const isEnglish = currentLanguage() === "en";
    setStatus(isEnglish ? `Checking local helper: ${LOCAL_HELPER_URL}` : `正在检测本地助手：${LOCAL_HELPER_URL}`);
    setProgress(8, isEnglish ? "Checking helper" : "检测本地助手");

    try {
      const result = await requestLocalHelperStatus();
      const ffmpegStatus = result.ffmpegStatus || {};
      const fixes = Array.isArray(ffmpegStatus.fix) ? ffmpegStatus.fix : [];
      if (isEnglish) {
        setStatus([
          "Local helper is online.",
          `Version: ${result.version || "unknown"}`,
          `Download folder: ${result.downloadDir || "Windows Downloads\\BiliFetch"}`,
          `ffmpeg: ${ffmpegStatus.message || (result.ffmpeg ? "ready" : "unknown")}`,
          fixes.length ? `Fix: ${fixes.join(" ")}` : "",
          "You can click Start now. Keep the helper window open while downloading.",
        ].filter(Boolean).join("\n"));
      } else {
        setStatus([
          "本地助手在线。",
          `版本：${result.version || "未知"}`,
          `下载目录：${result.downloadDir || "Windows 下载目录\\BiliFetch"}`,
          `ffmpeg：${ffmpegStatus.message || (result.ffmpeg ? "可用" : "状态未知")}`,
          fixes.length ? `修复建议：${fixes.join(" ")}` : "",
          "现在可以点击“开始”。下载期间请保持本地助手窗口打开。",
        ].filter(Boolean).join("\n"));
      }
      setProgress(100, isEnglish ? "Helper online" : "本地助手在线");
    } catch (error) {
      if (isEnglish) {
        setStatus([
          "Local helper was not detected.",
          `Reason: ${error.message}`,
          "Fix: unzip the beginner package first, double-click the local helper startup bat, keep its window open, then click Check Helper again.",
          "You can still click Start; the script will try browser-side merging if possible.",
        ].join("\n"));
      } else {
        setStatus([
          "没有检测到本地助手。",
          `原因：${error.message}`,
          "处理方法：先解压小白包，双击“启动BiliFetch本地助手.bat”，保持窗口打开，然后再点“检测助手”。",
          "不启动助手也可以点“开始”；脚本会在可行时尝试浏览器内合并。",
        ].join("\n"));
      }
      setProgress(0, isEnglish ? "Helper not detected" : "未检测到本地助手");
    }
  }
  ...
  function requestLocalHelperStatus() {
    const url = `${LOCAL_HELPER_URL}/status`;
  ...
  if (typeof GM_registerMenuCommand === "function") {
    ...
    GM_registerMenuCommand("检测 BiliFetch 本地助手", () => checkLocalHelperStatus().catch(showError));
